...
from qvic.constants import LOGDIR
...
def print_trainable_parameters(model):
    ...
    all_param = sum(p.ds_numel if hasattr(p, "ds_numel") else p.numel() for p in model.parameters())
    trainable_parameters = sum(p.ds_numel if hasattr(p, "ds_numel") else p.numel() for p in model.parameters() if p.requires_grad)
    #rank0_print(f"Total parameters: ~{total_params/1e6:.2f} MB)")
    #rank0_print(f"Trainable parameters: ~{trainable_params/1e6:.2f} MB)")            
            
    if all_param > 0:
        print(f"trainable params: {trainable_parameters/1e6:.2f} MB || all params: {all_param/1e6:.2f} MB || trainable: {100 * trainable_parameters / all_param: .2f} %")
    else:
        print(f"trainable params: {trainable_parameters/1e6:.2f} MB || all params: {all_param/1e6:.2f} MB || trainable: N/A")
