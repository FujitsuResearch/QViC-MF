...
import torch
from qvic.model import *
from qvic.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from qvic.utils import rank0_print
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if ("llava" in model_name.lower() or "qvic" in model_name.lower()) or is_multimodal:
        # Load LLaVA model
        if ("lora" in model_name.lower() or "qvic" in model_name.lower()) and model_base is None:
            warnings.warn(
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                from qvic.model.language_model.llava_mixtral import LlavaMixtralConfig
                ...
                from qvic.model.language_model.llava_mistral import LlavaMistralConfig
                ...
                from qvic.model.language_model.llava_gemma import LlavaGemmaConfig
                ...
                from qvic.model.language_model.llava_llama import LlavaConfig
        ...
        # === QViC modification (Fujitsu, CC BY-NC-SA 4.0): QViC LoRA loading branch ===
        # Loads the QViC base model, QViC context embedding (non-lora trainables),
        # the dedicated context encoder, and the QViC LoRA adapters. This is the
        # single biggest QViC-specific block in this file; all upstream LLaVA-NeXT
        # branches above and below remain untouched.
        elif "qvic" in model_name.lower() and model_base is not None:
            ...
            rank0_print(f'(LoRA) Loading QViC base model from {model_base}')
            is_qwen_base = "qwen" in model_name.lower() or (model_base is not None and "qwen" in model_base.lower())
            if is_qwen_base:
                # [BUG?] correct vocab_size
                # ref. https://github.com/LLaVA-VL/LLaVA-NeXT/issues/329
                cfg_pretrained.vocab_size = 152064
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            ...
            rank0_print(f'(LoRA) Loading context embedding from {model_path}')
            model.initialize_context_embed(cfg_pretrained)
            non_lora_trainables = torch.load(os.path.join(model_path, 'non_lora_trainables.bin'), map_location='cpu')
            non_lora_trainables = {k.replace('base_model.model.context_embed.', 'context_embed.'): v for k, v in non_lora_trainables.items()}
            if len(non_lora_trainables) == 0:
                rank0_print(f'[WARNING] No non-lora trainables found in {model_path}. If avepool, ignore this.')
                breakpoint()
            ret = model.load_state_dict(non_lora_trainables, strict=False)
            if len(ret.unexpected_keys) > 0:
                raise ValueError(f"Unexpected keys in loading rola trainables: {ret.unexpected_keys} from {model_path}")

            rank0_print(f'(LoRA) Loading encoder/decoder LoRA from {model_path}')

            if model.question_guided_selective_attention:
                attn_implementation_encoder = "sdpa"  # for Question-Guided Selective Attention
            else:
                attn_implementation_encoder = attn_implementation
            rank0_print(f"Using attention implementation: {attn_implementation_encoder}")

            if is_qwen_base:
                from qvic.model.language_model.modeling_qwen2 import Qwen2ForCausalLM
                model.encoder = Qwen2ForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, attn_implementation=attn_implementation_encoder, **kwargs)
            else:
                from transformers import LlamaForCausalLM
                model.encoder = LlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, attn_implementation=attn_implementation_encoder, **kwargs)

            assert os.path.exists(os.path.join(model_path, 'adapter_model.bin'))
        ...
        # === end QViC modification ===
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    from qvic.model.language_model.llava_qwen_moe import LlavaQwenMoeConfig
                    if overwrite_config is not None:
                    ...
                    from qvic.model.language_model.llava_qwen import LlavaQwenConfig
                    if overwrite_config is not None:
